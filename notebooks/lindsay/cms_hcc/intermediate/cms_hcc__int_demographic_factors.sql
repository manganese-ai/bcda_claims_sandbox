...
-- get info from previous script, cms_hcc__int_members
with members as (
...
-- has different coeffiecients for combinations of gender, age group, medicaid status, OREC, etc
, seed_demographic_factors as (
    ...
    select
        ...
        , institutional_status
        , coefficient
    from {{ ref('cms_hcc__demographic_factors') }} -- this is a seed file
...
-- combine coefficient data and patient data for continuing enrollees
-- same as new enrollees except also including Dual Status and Institutional Status for Continuing (not New)
