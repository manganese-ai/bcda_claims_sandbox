...
-- pull in seed data that has adjustment factors based on the payment year (to normalize costs year to year?)
...
-- calculate the raw risk score for each patient by summing up all the coefficients for that payment year
, raw_score as (
...
-- calculate the normalized risk score for each patient (for that payment year) by dividing the raw risk score by the normalization factor for that payment year
, normalized as (
...
-- calculate the payment risk score for each patient (for that payment year) by multiplying the normalized risk score by (1-adjustment), where the adjustment is set by MA.
-- the adjustment is the same for each year in our data, so I'm not sure what it's doing.
, payment as (
