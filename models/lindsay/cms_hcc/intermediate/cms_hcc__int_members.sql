...
/* VARIABLES
For our data:
    - `payment_year_age_date` could be changed to '2019-02-01'
    - `collection_year_start` could be changed to '2018-01-01'
    - `collection_year` could be changed to 2018
*/
...
-- find all enrollments for each patient (1 = most recent, n = least recent)
with stg_eligibility as (
    ...
    select
        ...
        , gender
        , birth_date
        , floor({{ datediff('birth_date', "'"~payment_year_age_date~"'", 'hour') }} / 8766.0) as payment_year_age -- Age is calculated as of Feb 1 of the payment year
        ...
        ) as row_num -- used to dedupe eligibility: for each patient, the number of enrollments is tallied from 1 to n
    from {{ ref('cms_hcc__stg_eligibility') }}

)

-- define the number of months covered during the collection year
-- only include rows where the enrollment was during the collection year
, calculate_prior_coverage as (
    ...
    (extract(year from enrollment_start_date) = {{ collection_year }}
     or extract(year from enrollment_start_date) = {{ collection_year }}) /* THIS IS WHERE I THINK THE ERROR IS */ 

)

-- for each row, define whether an enrollment status is new or continuing
-- via CMS: New Enrollee is when a beneficiary has < 12 months of coverage PRIOR to the payment year
, add_enrollment as (
...
, latest_eligibility as (
    ...
    select
        ...
        , case
          ...
          end as enrollment_status_default -- True if there's a problem, False if not
    from stg_eligibility
         left join add_enrollment
         on stg_eligibility.patient_id = add_enrollment.patient_id
    where stg_eligibility.row_num = 1 -- most recent eligibility for each patient

)

-- add age information. Same for Continuing and New except for ages 65-69.
-- Not sure why it is more granular for New patients instead of continuing
, add_age_group as (
...
-- cast cols to correct data type
, add_data_types as (
    ...
    select
        ...
        , cast(case
          ...
          end as {{ dbt.type_string() }}) as medicaid_status -- SHOULD THIS BE MEDICARE INSTEAD OF MEDICAID? 
        , cast(case
